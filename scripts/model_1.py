...
local_server = "http://localhost:123/v1"
...
# Load the dataset
try:
    df_clone = pd.read_csv('../dataset/parallel/processing/sms_predictions_1_in_progress.csv')
    print("Resuming from saved progress.")
except FileNotFoundError:
    df = pd.read_csv('../dataset/parallel/sms_1.csv')
    df_clone = df.copy()
...
for index, row in df_clone.iterrows():
    if pd.isnull(row['Predicted']):
        ...
        df_clone.to_csv('../dataset/parallel/processing/sms_predictions_1_in_progress.csv', index=False)
...
df_clone.to_csv('../dataset/parallel/results/sms_predictions_1_results.csv', index=False)
